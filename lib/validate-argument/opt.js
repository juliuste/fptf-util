...
const validateOptWhen = w => {
    if (!isNull(w) && !isUndefined(w)) {
        const message = `opt.when must be a date object or null/undefined`
...
const validateOptVia = v => {
    if (!isNull(v) && !isUndefined(v)) {
        const validateStation = validateRequiredArgument.createValidateStation('opt.via')
        validateStation(v)
    }
    return true
}
